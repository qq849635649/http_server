...
#define ncx_align_ptr(p, a) (((uintptr_t) (p) + ((uintptr_t) a - 1)) & ~((uintptr_t) a - 1))
...
ncx_slab_init(ncx_slab_pool_t * pool)
{
	...
	for (n = ncx_pagesize, ncx_pagesize_shift = 0; n >>= 1;
	     ncx_pagesize_shift++)
	{/* void */}
	...
	size = pool->end - p;
	pages = (ncx_uint_t) (size /
			      (ncx_pagesize + sizeof (ncx_slab_page_t)));
	...
	pool->start = (u_char *)
		ncx_align_ptr ((uintptr_t) p +
			       pages * sizeof (ncx_slab_page_t),
			       ncx_pagesize);
...
ncx_slab_alloc_locked (ncx_slab_pool_t * pool, size_t size)
{
	...
	{
		page = ncx_slab_alloc_pages (pool,
	...
	if (page->next != page)
	{
		...
		{		/* shift > ncx_slab_exact_shift */
			...
			n = ncx_pagesize_shift -
				(page->slab & NCX_SLAB_SHIFT_MASK);
			n = 1 << n;
			n = ((uintptr_t) 1 << n) - 1;
		     ...
		     ncx_uint_t pages)
{
	...
	{
		ncx_memzero (&page[1],
			     (pages - 1) * sizeof (ncx_slab_page_t));
	}
...
ncx_slab_stat (ncx_slab_pool_t * pool, ncx_slab_stat_t * stat)
{
	...
	for (i = 0; i < stat->pages; i++)
	{
		...
		case NCX_SLAB_BIG:
			...
			for (m = (uintptr_t) 1 << NCX_SLAB_MAP_SHIFT;
			     m & mask; m <<= 1)
			{
				if ((page->slab & m))
				{
	...
	info ("small slab use page : %zu,\tbytes : %zu", stat->p_small,
	      stat->b_small);
	info ("exact slab use page : %zu,\tbytes : %zu", stat->p_exact,
	      stat->b_exact);
	info ("big   slab use page : %zu,\tbytes : %zu", stat->p_big,
	      stat->b_big);
