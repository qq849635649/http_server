...
MD5 & MD5::GenerateMD5(const string & input)
{
	struct md5_context context;
	md5_starts(&context);
	md5_update(&context, reinterpret_cast<uint8_t*>(const_cast<char *>(input.data())), input.size());
...
string MD5::toString(void)
{
	char output[33] = {};
	for(int j = 0; j < 16; j++ )
		sprintf(output + j * 2, "%02x", ((uint8_t *)m_data)[j]);
