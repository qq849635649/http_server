...
int get_md5(char* src,char* dst)
{
    ...
    MD5_CTX md5;
    MD5Init(&md5);
    MD5Update(&md5,psrc,strlen(src));
    MD5Final(&md5,decrypt);
    memcpy(dst,decrypt,16);
/*
    for(i=0;i<16;i++)
    {
        memset(tmp,0,10);
...
int get_md5_withlen(char* src,int len,unsigned char* dst)
{
    ...
    MD5_CTX md5;
    MD5Init(&md5);
    MD5Update(&md5,psrc,len);
    MD5Final(&md5,decrypt);
    memcpy((char*)dst,decrypt,16);
/*
    for(i=0;i<16;i++)
    {
        memset(tmp,0,10);
...
int get_md5_32_withlen(char* src,int len,char* dst,int dstlen)
{
    ...
    for(i=0;i<16;i++)
    {
        ret+=snprintf(dst+ret,dstlen-ret,"%02x",decrypt[i]);
    }
    return ret;
}
/*
...
06.{  
07.    int i;  
08.    unsigned char encrypt[] ="admin";//21232f297a57a5a743894a0e4a801fc3  
09.    unsigned char decrypt[16];      
10.    MD5_CTX md5;  
11.    MD5Init(&md5);                
